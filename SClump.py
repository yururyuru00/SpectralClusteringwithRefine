...
def spectral_clustering(S, labels, iter):
    if(P.disable_normalization == False):
...
def ini_lambda(sp):
    ...
    if(P.dataset_type == 'social_net'):
        ...
        with open('./data/{0}/{0}_pca_contribution_ratio.csv'.format(P.dataset), 'r') as r:
            ...
            pca_contribution_ratio = np.array(
                [float(val) for val in pca_contribution_ratio])
...
def update_S(S, W, vec):
    ...
    for i in range(len(S)):
        ...
        ui = pi + 1./P.number_of_updates*v_1 - \
            np.dot(v_1, pi)/P.number_of_updates*v_1
...
def main():
    ...
    parser.add_argument(
        'dataset', type=str, help='any of the following datasets {cora, citeseer, football, politicsuk, olympics}')
    ...
    np.random.seed(0)
    if(P.dataset == 'cora'):
        features, edges, labels = utilities.load_fromgen(P.dataset)
    else:
        features, edges, labels = utilities.load_fromcsv(P.dataset)
    ...
    # Spectral Clustering + Refine(update S, lambda, W)
    mean_silhouette_score = np.zeros(100)
    for tri in range(100):
        pred_clusters, eigen_val, eigen_vec, metrics = spectral_clustering(
            S, labels, tri)

        print("tri: {}\n\tARI: {:.4f}\n\tNMI: {:.4f}\n\tPurity: {:.4f}"
              .format(tri, metrics['ari'], metrics['nmi'], metrics['purity']))
        number_of_eigen0 = gamma_tuning(eigen_val)
        print("\tnum of components: {}".format(number_of_eigen0))

        # stopping criteria
        mean_silhouette_score[tri] = utilities.calc_silhouette_score(
            P.dataset, pred_clusters)
        print('\tmean silhouette score : {}\n'.format(
            mean_silhouette_score[tri]))
        if(tri >= 2 and mean_silhouette_score[tri-2] < mean_silhouette_score[tri-1] > mean_silhouette_score[tri]):
            print('refine process ended at iter #{} by stopping criteria'.format(tri-1))
            break

        # converge criteria
        if(number_of_eigen0 == P.cluster_size):
            print('refine process ended at iter #{} by converge criteria'.format(tri))
            break

        np.savetxt('./result/{}/pred_clusters#{}.csv'.format(P.dataset, tri),
                   pred_clusters, fmt='%d')
