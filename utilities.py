...
def load_fromgen(dataset):
    idfeature_features_labels = np.genfromtxt("./data/{0}/{0}.content".format(dataset),
                                              dtype=np.dtype(str))
    features = np.array([[float(i) for i in vec]
                         for vec in idfeature_features_labels[:, 1:-1]])
    ...
    edges_unordered = np.genfromtxt(
        "./data/{0}/{0}.cites".format(dataset), dtype=np.int32)
...
def make_sp1(features):
    def makeMat_sp1_i(feature, usr_size):
        ...
        for i in range(usr_size):
            for j in range(i+1, usr_size):
                mat[i][j] = math.exp(-(feature[i]-feature[j])
                                     * (feature[i]-feature[j]) / (2*P.sigma*P.sigma))
                mat[j][i] = mat[i][j]
    ...
    if(P.dataset_type == 'social_net'):  # AN_type=human
        ...
        for i in range(usr_size):
            for j in range(i+1, usr_size):
                mat[i][j] = np.dot(features[i], features[j]) / \
...
class Graph():
    def __init__(self, edges, usr_size):
        ...
        self.R = np.array([[set() for i in range(usr_size)]
                           for j in range(P.theta+1)])
        self.Ft = np.array([[set() for i in range(usr_size)]
                            for j in range(P.theta+1)])
    ...
    def makeMat_sp2(self):
        ...
        for step in range(2, P.theta+1):
            for v in range(len(self.N)):
                ...
                self.max_size[step] = max(
                    self.max_size[step], len(self.Ft[step][v]))
...
def calc_silhouette_score(dataset, pred_clusters):
    if(dataset == 'cora'):
        idfeature_features_labels = np.genfromtxt("./data/{0}/{0}.content".format(dataset),
                                                  dtype=np.dtype(str))
        features = np.array([[float(i) for i in vec]
    ...
    if(dataset == 'citeseer'):
        with open("./data/{0}/{0}.content".format(dataset), 'r') as r:
            [dim_size, obj_size] = list(
                map(int, r.readline().rstrip().split(' ')))
        features = np.zeros((obj_size, dim_size), dtype=np.float)
        dim_idobjct_val = np.genfromtxt("./data/{0}/{0}.content".format(dataset),
                                        skip_header=1, dtype=(int, int, float))
        for [dim_id, obj_id, val] in dim_idobjct_val:
    ...
    if(dataset in ['football', 'politicsuk', 'olympics']):
        with open("./data/{0}/{0}_tfidf.content".format(dataset), 'r') as r:
            [dim_size, obj_size] = list(
                map(int, r.readline().rstrip().split(' ')))
        features = np.zeros((obj_size, dim_size), dtype=np.float)
        dim_idobjct_val = np.genfromtxt("./data/{0}/{0}_tfidf.content".format(dataset),
                                        skip_header=1, dtype=(int, int, float))
        for [dim_id, obj_id, val] in dim_idobjct_val:
            features[obj_id][dim_id] = float(val)

    silhouette_scores = silhouette_samples(
        features, pred_clusters, metric='cosine')
